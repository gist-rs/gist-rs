...
pub enum TokenError {
    ...
    Client(solana_web3_wasm::solana_client_wasm::ClientError),
...
pub type TokenResult<T> = anyhow::Result<T, TokenError>;
...
pub struct Token<T> {
    client: WasmClient,
    // _client: Arc<dyn ProgramClient<T>>,
    foo: Option<T>,
    ...
    T: SendTransaction,
{
    ...
    pub fn new_native(client: WasmClient, program_id: &Pubkey, payer: Arc<dyn Signer>) -> Self {
        Self::new(
            client,
            program_id,
